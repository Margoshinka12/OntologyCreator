...
namespace OntologyCreator.Forms
{
    public partial class MainForm : Form
    {
        ...
        private void btnInfoAttribute_Click(object sender, EventArgs e)
        {
            Property prop = activeConcept.Properties.Find(p => p.ID == (int)dgvAttributes.SelectedRows[0].Cells["PropertyID"].Value);
            var property = new PropertyForm(3, currentOntology.Id, activeConcept, prop);
            this.Enabled = false;
            property.Show();
            property.Closed += ChildForm_Closed;
        }
        ...
        private void dgvAttributes_SelectionChanged(object sender, EventArgs e)
        {
            if ((dgvAttributes.SelectedRows.Count != 0) && (dgvAttributes.SelectedRows[0].Cells["PropertyID"].Value != null))
                lblDescription.Text = activeConcept.Properties.Find(p => p.ID == (int)dgvAttributes.SelectedRows[0].Cells["PropertyID"].Value).Description;
            AttributesEnable();
        }
        ...
        {
            var relation = new RelationForm(2, currentOntologyId, activeConcept,
                currentOntology.Relations.Find(r => r.ID == (int)dgvRelations.SelectedRows[0].Cells["RelationID"].Value));
            this.Enabled = false;
            relation.Show();
            relation.Closed += ChildForm_Closed;
        }

        private void btnInfoRelation_Click(object sender, EventArgs e)
        {
            var relation = new RelationForm(3, currentOntologyId, activeConcept,
                currentOntology.Relations.Find(r => r.ID == (int)dgvRelations.SelectedRows[0].Cells["RelationID"].Value));
            this.Enabled = false;
            relation.Show();
            relation.Closed += ChildForm_Closed;
        ...
        private void dgvRelations_SelectionChanged(object sender, EventArgs e)
        {
            if ((dgvRelations.SelectedRows.Count != 0) && (dgvRelations.SelectedRows[0].Cells["RelationID"].Value != null))
                lblDescription.Text = currentOntology.Relations.Find(r => r.ID == (int)dgvRelations.SelectedRows[0].Cells["RelationID"].Value).Description;
            RelationsEnable();
        }
        ...
        private void FillTheWindow()
        {
            if (activeConcept != null)
            {
                lblDescription.Text = activeConcept.Description;
        ...
        private void AttributesEnable()
        {
            ...
            {
                btnInfoAttribute.Enabled = true;
                btnEditAttribute.Enabled = true;
                btnDelAttribute.Enabled = true;
            }
            else
            {
                btnInfoAttribute.Enabled = false;
        ...
        private void RelationsEnable()
        {
            if ((activeConcept != null) && (dgvRelations.SelectedRows.Count != 0))
            {
                ...
                {
                    btnInfoRelation.Enabled = false;
                    btnEditRelation.Enabled = false;
                    btnDelRelation.Enabled = false;
                }
                else
                {
                    btnInfoRelation.Enabled = true;
        ...
        private void FillInterview()
        {
            if (currentOntology.Concepts.Count == 0)
                lblInterview.Text = "Для начала создайте главную сущность Вашей предметной области. Затем к ней можно будет добавить подклассы и свойства." +
                    " Для добавления сущности (класса) нажмите на кнопку \"Добавить класс\" и следуйте дальнейшим инструкциям.";
            if (currentOntology.Concepts.Count == 1)
                lblInterview.Text = "Для построения связей между концептами, отличных от наследования необходимо добавить ещё классов." +
                    " Для добавления сущности (класса) нажмите на кнопку \"Добавить класс\" и следуйте дальнейшим инструкциям.";
            if (currentOntology.Concepts.Count > 1)
                lblInterview.Text = "Так как у Вас есть несколько главных классов, вы можете добавить связь между ними и их подклассами." +
                    " Для добавления связи (отношения) нажмите на кнопку \"Добавить связь\" и следуйте дальнейшим инструкциям.";
            if (activeConcept != null)
            {
                if (activeConcept.Properties.Count == 0)
                    lblInterview.Text = "У выбранного класса нет свойств. Если хотите, то можете их добавить" +
                         " Для добавления свйоства для класса нажмите на кнопку \"Добавить свойство\" и следуйте дальнейшим инструкциям.";
                if (activeConcept.ParentID == -1)
                    lblInterview.Text = "Сейчас, если нужно, можно добавить подклассы к выбранному главному классу." +
                         " Для добавления подкласса нажмите на кнопку \"Добавить подкласс\" и следуйте дальнейшим инструкциям.";
            }
        }
